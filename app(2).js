...
async function togglePause(id, currentlyPaused) {
  await updateDoc(doc(db, "goals", id), { paused: !currentlyPaused });
}
...
function listenNotes() {
  onSnapshot(query(collection(db, "notes"), where("ownerUid", "==", currentUid)), (snap) => {
    notes = snap.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => a.dateISO.localeCompare(b.dateISO));
    if (activeTab === "progresso") renderProgresso();
  });
}
async function addNote(text, tag) {
  await addDoc(collection(db, "notes"), { ownerUid: currentUid, text, tag: tag || null, dateISO: todayISO(), createdAt: serverTimestamp() });
}
...
// Histórico do mês corrente de verdade: do dia 1 até hoje (não uma janela rolante de N dias,
// que fazia dias de meses anteriores vazarem pra dentro da "semana atual").
...
// Agrupa os dias do mês em semanas de calendário reais (segunda a domingo),
// então a "semana 1" pode ter menos de 7 dias se o mês não começar numa segunda.
function buildCalendarWeeks(monthHistory) {
  const groups = new Map();
  monthHistory.forEach((day) => {
    const key = mondayOfWeek(day.dateISO);
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(day);
  });
  return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([, days]) => days);
}
...
function renderProgresso() {
  const history = buildHistory(28); // janela rolante, só pra calcular o streak (não depende do mês-calendário)
  const currentStreak = computeCurrentStreak(history);
  const longestStreak = computeLongestStreak(history);

  const monthHistory = buildMonthHistory(); // dia 1 do mês corrente até hoje
  const weeks = buildCalendarWeeks(monthHistory);
  ...
  weeks.forEach((chunk, i) => {
    ...
    const isCurrent = chunk.some((d) => d.dateISO === todayISO());
    const label = `Sem ${i + 1}${isCurrent ? " (atual)" : ""}`;
    html += `
  ...
  if (notesToggle) notesToggle.addEventListener("click", () => { notesExpanded = !notesExpanded; renderProgresso(); });
  const btnAddNote = document.getElementById("btn-add-note");
  if (btnAddNote) btnAddNote.addEventListener("click", (e) => { e.stopPropagation(); openNoteSheet(); });
  tabContent.querySelectorAll("[data-del-note]").forEach((btn) => {
    btn.addEventListener("click", (e) => { e.stopPropagation(); deleteNote(btn.dataset.delNote); });
  });

  tabContent.querySelectorAll("[data-week]").forEach((btn) => {
    btn.addEventListener("click", () => openWeekSheet(weeks[Number(btn.dataset.week)]));
  });
  document.getElementById("btn-month-summary").addEventListener("click", () => openMonthSummarySheet(monthHistory));
}
...
// ---------- seção Anotações (diário rápido, com contagem semanal por categoria) ----------

function mondayOfWeek(dateISO) {
  const d = new Date(dateISO + "T00:00:00");
  const day = d.getDay(); // 0 = domingo
  const diff = day === 0 ? -6 : 1 - day; // volta pra segunda-feira dessa semana
  d.setDate(d.getDate() + diff);
  return isoFromDate(d);
}

function weeklyTagCounts(entries) {
  const monday = mondayOfWeek(todayISO());
  const counts = {};
  entries.forEach((n) => {
    if (!n.tag || n.dateISO < monday) return;
...
function notesSectionHTML() {
  const chevron = notesExpanded ? "▲" : "▼";
  const tagCounts = weeklyTagCounts(notes);
  const tagSummary = Object.entries(tagCounts)
    .map(([tag, count]) => `${escapeHTML(tag)}: ${count}x essa semana`)
    .join(" · ");
    ...
    inner += `
      <div style="margin-top:14px;">
        <div class="row-header" style="margin-bottom:8px;">
          <span class="section-label" style="margin:0;">Últimas anotações</span>
          <button class="btn-icon" id="btn-add-note" style="width:26px; height:26px; font-size:14px; border-radius:8px;">+</button>
        </div>
        ${notes.length ? notes.slice().reverse().slice(0, 15).map((n) => `
          <div style="display:flex; align-items:flex-start; gap:10px; padding:8px 0; border-bottom:1px solid var(--border);">
            <div style="flex:1;">
              <div style="font-size:13px;">${escapeHTML(n.text)}</div>
              <div style="font-size:11px; color:var(--muted); margin-top:2px;">${formatDateBR(n.dateISO)}${n.tag ? ` · ${escapeHTML(n.tag)}` : ""}</div>
            </div>
            <button data-del-note="${n.id}" style="background:transparent; border:none; color:var(--muted); font-size:14px; cursor:pointer; padding:2px;">✕</button>
          </div>
        `).join("") : `<div class="empty-msg" style="padding:16px 0;">Nenhuma anotação ainda.</div>`}
      </div>
    `;
  }

  return `
    <div class="goal-card" style="flex-direction:column; align-items:stretch; cursor:pointer; margin-top:10px;" id="notes-toggle">
      <div style="display:flex; align-items:center; gap:10px;">
        <div class="goal-title">Anotações</div>
        <div class="goal-time">${tagSummary || "diário rápido"}</div>
  ...
  overlay.innerHTML = `
    <div class="sheet">
      <div class="sheet-header">
        <span class="sheet-title">NOVA ANOTAÇÃO</span>
        <button class="sheet-close" id="close-sheet-note">✕</button>
      </div>
      <div class="field-group">
        <div class="field-label">O que aconteceu</div>
        <input type="text" id="f-note-text" placeholder="Ex: comi besteira no lanche" />
      </div>
      <div class="field-group">
        <div class="field-label">Categoria (opcional)</div>
        <input type="text" id="f-note-tag" placeholder="Ex: besteira" />
        <div class="hint">Anotações com a mesma categoria são contadas por semana ali em cima.</div>
      </div>
      <button class="save-btn" id="save-note">Salvar anotação</button>
    </div>
  `;
  document.body.appendChild(overlay);
  overlay.querySelector("#close-sheet-note").addEventListener("click", () => overlay.remove());
  overlay.querySelector("#save-note").addEventListener("click", async () => {
    const text = overlay.querySelector("#f-note-text").value.trim();
    if (!text) return;
    const tag = overlay.querySelector("#f-note-tag").value.trim().toLowerCase();
    await addNote(text, tag || null);
    overlay.remove();
  });
}
...
function renderMetas() {
  ...
  tabContent.querySelectorAll("[data-pause]").forEach((btn) => {
    btn.addEventListener("click", () => togglePause(btn.dataset.pause, btn.dataset.paused === "true"));
  });
}

function goalCardHTML(g, isAvulso) {
  return `
    <div class="goal-card${isAvulso ? " avulso" : ""}"${g.paused ? ' style="opacity:.5;"' : ""}>
      <div class="goal-title">${escapeHTML(g.title)}${g.paused ? ' <span style="color:var(--muted); font-size:11px;">(pausada)</span>' : ""}</div>
      <div class="goal-time">${goalTimeLabel(g)}</div>
      <button class="goal-edit" data-pause="${g.id}" data-paused="${!!g.paused}">${g.paused ? "▶" : "⏸"}</button>
